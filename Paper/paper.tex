...
\begin{abstract}
  ...
  The jet precesses along a biconical surface due to the binary's
  orbital motion.  When the binary enters the GW phase of its
  ...
  due to the superluminal motion.  For binaries with mass
  $10^7$--$10^{10}$ M$_\odot$ at redshifts $z<0.5$, monitoring these
  features in current and archival data will place a lower limit on
  sources that could be detected by eLISA and Pulsar Timing Arrays.
...
binary members during its final phase of evolution before coalescence,
...
As the binary evolves, the combination of increasing orbital period
and speed, and the apparant superluminal motion effect, results in a
jet morphology with an increasing opening angle and increasingly rapid
...
streches on the sky while the backward jet is compressed due to the
...
its GW phase. Observations of such a milliarcsecond geometry can thus
...
inspiral exhibits a distinct chirping morphology. Jets in these
binaries have a biconical morphology.  After the binary enters the GW
phase, the opening angle of cone rapidly increases within a separation
of tens of milli-arcseconds at a distance of 100~Mpc.  In addition,
the periodicity of the jet's motion on the conical surface increases
as the orbital period of the binary goes up in this stage of its
evolution.  Finally, due to the apparent superluminal motion and the
widening opening angle, the jet is twisted into a peculiar morphology.
The jet geometry can be used to infer the set of dynamical and
geometrical parameters, listed in Table~\ref{table:params}.
...
We make the code for implementing the model presented in this Letter
